...
### 3. SUBSET DATA
      d_1 <- df %>% filter(Rapporteringsniva=="Helgeland 1", Aar==2021)
...
tsTotPlot = ggplot(data = timeSeriesTot, aes(x=År, y=Estimat, group = Faktor)) + 
  ...
  labs(title = "Helgeland 1", subtitle = bquote("Total tetthet - Antall fugl/"*km^2)) + 
  xlab("År") + 
  ylab(bquote(phantom(0)["Tetthet ("%+-%"SE)"])) + 
  geom_ribbon(aes(ymin = `Nedre CL`, ymax = `Øvre CL`), alpha = 0.1, fill = "steelblue") + theme(axis.title.y = element_text(size = 14)) 

tsAdPlot = ggplot(data = timeSeriesAd, aes(x = År, y = Estimat, group = Faktor)) + 
  geom_line(color = "black", size = 1, alpha = 1, linetype = 1) +
  labs(title = "Helgeland 1 ", subtitle = bquote("Voksen tetthet - Antall fugl/"*km^2)) +
  xlab("År") +
  ylab(bquote(phantom(0)["Tetthet ("%+-%"SE)"])) +
  geom_ribbon(aes(ymin = `Nedre CL`, ymax = `Øvre CL`), alpha = 0.1, fill = "steelblue") +
  theme(axis.title.y = element_text(size = 14))

tsChickPlot = ggplot(data = timeSeriesChick, aes(x = År, y = Estimat, group = Faktor)) + 
  geom_line(color = "black", size = 1, alpha = 1, linetype = 1) + 
  labs(title = "Helgeland 1", subtitle = bquote("Kylling tetthet - Antall fugl/"*km^2)) + 
  xlab("År") + 
  ylab(bquote(phantom(0)["Tetthet ("%+-%"SE)"])) +
  geom_ribbon(aes(ymin = `Nedre CL`, ymax = `Øvre CL`), alpha = 0.1, fill = "steelblue") +
  theme(axis.title.y = element_text(size = 14))
...
tsRodentsPlot = ggplot(data = timeSeriesRodents, aes(x = Year, y = SeeRodPercent, group = 1)) + 
  geom_line(color = "black", size = 1, alpha = 1, linetype = 1) +
  labs(title = "Helgeland 1", subtitle = bquote("Smågnager observasjoner")) +
  xlab("År") +
...
ggsave(filename = "C:/Users/markus.israelsen/OneDrive - NINA/GitHub/HonsefuglReport/Honsefugl-Report/Figure/Figure1.jpg")

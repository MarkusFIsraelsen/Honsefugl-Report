...
```{r 3 - NINA & HINN, echo=FALSE, message=FALSE}
    ...
    ### 14A: ESTIMATING BROOD SIZE POOLED
      Rekrutt_all <- rec(strat="OmradeNavn")
...
**Tabell 1**. Deskriptiv statistikk for `r paste(d$Navn[1])` i
`r paste(d$Year[1])`.
...
med nedre og øvre CL (konfidensintervall). Den «sanne» tettheten ligger
et sted mellom nedre og øvre CL, noe som bør tas i betraktning i
...
**Tabell 2**. Resultater for `r paste(Art2)` i `r paste(d$Navn[1])` i
`r paste(d$Year[1])`. Kyllingproduksjon rapporteres som antall kyllinger
per par (per høne for skogsfugl). Nedre CL og Øvre CL representerer
...
if(strat == "OmradeNavn"){
  ...
  for(i in 1:dim(areaName)[1]){
    if(any(is.na(areaName[i, ]))){
      areaName[i, ][which(is.na(areaName[i, ]))] = "-"
      areaName$Områdenavn[i] = "-"
    }
    
  }
  pander(areaName)
  #pander(omr_res(strat = strat))
} else{
  panderOptions("table.split.table", 120)
  panderOptions("table.alignment.default", "left")
  pander(samle_res())  
...
```{r 8 - NINA OG HINN, echo = FALSE, message = FALSE}
  ...
  strat <- "OmradeNavn"   # "Year", "OmradeNavn", eller "No"
...
av **figur 1**. Her vises også en indeks på smågnagerforekomst i samme
...
if(all(is.na(unique(timeSeriesDF$Estimat))) == TRUE){
 ...
 tsChickPlot = ggplot(data = timeSeriesChick, aes(x = Year, y = Estimat, group = Faktor)) + geom_point() +
     geom_line(color = "black", size = 1, alpha = 1, linetype = 1) +
     labs(title = ReportLevelOut, subtitle = bquote("c) Kyllingtetthet - Antall fugl/"*km^2)) +
     xlab("År") +
     ylab(bquote(phantom(0)["Tetthet (95% KI)"])) +
     geom_ribbon(aes(ymin = `Nedre CL`, ymax = `Øvre CL`), alpha = 0.4, fill = "steelblue") +
...
observasjoner / km^2^) for lirype, b) estimert voksen tetthet (antall
voksne fugler observert / km^2^) for lirype, c) estimert kylling tetthet
(antall kyllinger observert / km^2^) for lirype og til slutt d) antall
linjer med sett smågnagere / antall takserte linjer. Det blå feltet
rundt trendlinjen i figur a), b) og c) representerer 95%
konfidensintervallet til estimatet. \pagebreak
...
(dvs. en nedre og øvre CL som ligger nærmere estimatet). Merk at vi har

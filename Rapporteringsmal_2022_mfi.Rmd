...
### 2. LOAD AND FORMAT DATA SET
      ...
      # Hente ut hvilke linjer som er aktive
      #Ja, det hentes ut fra tabellen "Takseringslinje": sqlQuery(conn, paste("SELECT LinjeID, FK_OmradeID, STAsText, Linjenavn, Aktiv FROM Takseringslinje"))
...
### DATA FOR TIME SERIES FIGURE
  ...
  ### CREATE AN EMPTY TABLE TO SAVE ALL THE ESTIMATES TO
  ...
  for(i in 1:length(years)){
    ...
    # CHECK IF THE AREA HAD STARTED CENSUS IN 2013
    while(dim(d)[1] == 0){
      #counter = counter + 1
      #i = years[counter]
      i = i + 1
  ...
  # SUBSET DATA AND CREATE FIGURES FOR TIME SERIES DATA
  ...
  # HAVE TO CHECK THAT "SEEN RODENTS" IS CORRECT (WE WANT THE NUMBER OF OBS / NUMBER OF ACTIVE LINES)
  timeSeries$Year = as.character.Date(timeSeries$Year)
  timeSeries = timeSeries %>% mutate(NumLines = 1)
  timeSeriesRodents = timeSeries %>% group_by(Year) %>% summarise(SeeRodPercent = (sum(SettSMagnager)/sum(NumLines))*100)
  
  plotName = unique(d$Rapporteringsniva)
  tsTotPlot = ggplot(data = timeSeriesTot, aes(x=År, y=Estimat, group = Faktor)) + 
    geom_line(color="black", size=1, alpha=1, linetype=1) +
    labs(title = plotName, subtitle = bquote("Total tetthet - Antall fugl/"*km^2)) + 
    xlab("År") + 
    ylab(bquote(phantom(0)["Tetthet (95% KI)"])) + 
    geom_ribbon(aes(ymin = `Nedre CL`, ymax = `Øvre CL`), alpha = 0.4, fill = "steelblue") + theme(text = element_text(size = 8), axis.title.y = element_text(size = 12)) + ylim(0, ceiling(max(timeSeriesTot$`Øvre CL`, na.rm = TRUE))) 
  
  tsAdPlot = ggplot(data = timeSeriesAd, aes(x = År, y = Estimat, group = Faktor)) + 
    geom_line(color = "black", size = 1, alpha = 1, linetype = 1) +
    labs(title = plotName, subtitle = bquote("Voksen tetthet - Antall fugl/"*km^2)) +
    xlab("År") +
    ylab(bquote(phantom(0)["Tetthet (95% KI)"])) +
    geom_ribbon(aes(ymin = `Nedre CL`, ymax = `Øvre CL`), alpha = 0.4, fill = "steelblue") +
    theme(text = element_text(size = 8), axis.title.y = element_text(size = 12)) + ylim(0, ceiling(max(timeSeriesAd$`Øvre CL`, na.rm = TRUE)))
  
  tsChickPlot = ggplot(data = timeSeriesChick, aes(x = År, y = Estimat, group = Faktor)) + 
      geom_line(color = "black", size = 1, alpha = 1, linetype = 1) + 
      labs(title = plotName, subtitle = bquote("Kylling tetthet - Antall fugl/"*km^2)) + 
      xlab("År") + 
      ylab(bquote(phantom(0)["Tetthet (95% KI)"])) +
      geom_ribbon(aes(ymin = `Nedre CL`, ymax = `Øvre CL`), alpha = 0.4, fill = "steelblue") +
      theme(text = element_text(size = 8), axis.title.y = element_text(size = 12)) + ylim(0, ceiling(max(timeSeriesChick$`Øvre CL`, na.rm = TRUE))) 
  
  tsRodentsPlot = ggplot(data = timeSeriesRodents, aes(x = Year, y = SeeRodPercent, group = 1)) + 
    geom_line(color = "black", size = 1, alpha = 1, linetype = 1) +
    labs(title = plotName, subtitle = bquote("Smågnager observasjoner")) +
    xlab("År") +
  ...
  ggsave(filename = paste("C:/Users/markus.israelsen/OneDrive - NINA/GitHub/HonsefuglReport/Honsefugl-Report/Figure/", ReportLevelOut, ".jpg", sep = ""))

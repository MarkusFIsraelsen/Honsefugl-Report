...
```{r DATA IMPORT NINA, echo=FALSE, message=FALSE, warning=FALSE}
...
df = df %>% mutate(SettUkjentSmagnager = ifelse(SettUkjentSmagnager == "True", 1, 0), SettMus = ifelse(SettMus == "True", 1, 0), SettLemen = ifelse(SettLemen == "True", 1, 0)) 
df = df %>% mutate(SettSmagnager = c(SettUkjentSmagnager+SettMus+SettLemen))
...
### DATA FOR TIME SERIES FIGURE
  ...
  #                                                           ifelse(FK_ArtId==3, "Storfugl",
  #                                                                  ifelse(FK_ArtId==4, "Orrfugl", "Jerpe")))))
  #timeSeries = timeSeries %>% mutate(cs = AntallHann + AntallHunn + AntallKylling + AntallUkjent, År = Aar, Year = Aar, LinjeID = FK_LinjeID)
  ...
  # Total density plot
  tsTotPlot = ggplot(data = timeSeriesTot, aes(x=År, y=Estimat, group = Faktor)) + geom_point() +
  ...
  # Adult density plot
  tsAdPlot = ggplot(data = timeSeriesAd, aes(x = År, y = Estimat, group = Faktor)) + geom_point() +
    geom_line(color = "black", size = 1, alpha = 1, linetype = 1) +
  ...
  # Chick density plot
  tsChickPlot = ggplot(data = timeSeriesChick, aes(x = År, y = Estimat, group = Faktor)) + geom_point() +
      geom_line(color = "black", size = 1, alpha = 1, linetype = 1) + 

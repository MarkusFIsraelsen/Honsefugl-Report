...
ggplot(data = timeSeriesTot, aes(x=År, y=Estimat, group = Faktor)) + 
  geom_line( color="black", size=1, alpha=0.9, linetype=1) +
  labs(title = "Helgeland 1", subtitle = bquote("Total tetthet - Antall fugl/"*km^2)) + xlab("Year") + ylab(bquote(phantom(0)["Tetthet ("%+-%"SE)"])) + geom_ribbon(aes(ymin = `Nedre CL`, ymax = `Øvre CL`), alpha = 0.1) + theme(axis.title.y = element_text(size = 14))

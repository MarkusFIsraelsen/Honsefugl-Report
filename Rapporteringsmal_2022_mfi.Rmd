...
# ### 6. DEFINE NUMBER OF JUVENILES AND ADULTS; 
      ...
      d <- transform(d, ad=ifelse(Artnavn==Art1, ifelse(cs < 1.99, ad_t, 2), NA)) #dobbeltsjekk for at alle irrelevante observasjoner settes til NA
      d <- transform(d, juv=ifelse(Artnavn==Art1, ifelse(cs < 1.99, 0, cs - ad), NA)) #dobbeltsjekk for at alle irrelevante observasjoner settes til NA
...
```{r 6 - NINA & HINN, echo=FALSE, results='asis', message = FALSE}
...
if(all(is.na(unique(timeSeriesDF$Estimat))) == TRUE){
  ...
  tsChickPlot = ggplot(data = timeSeriesChick, aes(x=Year, y=Estimat, group = Faktor)) +
    labs(title = ReportLevelOut, subtitle = bquote("a) Kyllingtetthet - Antall fugl/"*km^2)) +
    xlab("År") +
    ylab(bquote(phantom(0)["Tetthet (95% CI)"])) + theme(text = element_text(size = 8), axis.title.y = element_text(size = 12)) + scale_y_continuous(breaks = integer_breaks(), limits = c(0, 10))
...
observasjoner / km^2^) for lirype, b) estimert voksentetthet (antall
voksne fugler observert / km^2^) for lirype, c) estimert kyllingtetthet
